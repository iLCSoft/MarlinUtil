...
int MarlinTrackFit::DoFitting(int useExtraPoint, int fitOpt, // inputs
  ...
  float chi2Sh;
  if (fitOpt == 0) { // simple helix fit with subroutine tfithl
    int IOPT = 2;
    // fit with simple helix hypothesis
    tfithl_(nhits,xd,yd,r,phi,wfr,zhit,
	    wfz,IOPT,param,eparam,chi2rphi,chi2z);    
    param[3] = param[3]*param[0]/fabs(param[0]);
  }
  else { // simple helix fit with ClusterShapes class
...
//   std::cout << " param before fit : omega = " << omega 
// 	    << " tl = " <<  tanlambda
// 	    << " phi = " << phi0
// 	    << " d0 = " << 10*d0
// 	    << " z0 = " << 10*z0
...
//   std::cout << "x,y,z = " << xInitial
// 	    << " " << yInitial
// 	    << " " << z0 << std::endl;
//   std::cout << "ref = " << ref[0] << " " << ref[1] << " " << ref[2] << " " 
// 	    <<  ref[3] << " " << ref[4] << " " << ref[5] << std::endl;

  int ierr;
  int fitCode;
  
  // And now fit using Kalman filter (FORTRAN routine trkfit based on DELPHI code)
  trackfit_(nfit,iDet,iTyp,xhit,yhit,zhit,rphireso,zreso,ref,ierr,rfit,rfite,chi2,ndf,fitCode);  
...
//       std::cout << "CovMatrix (DELPHI FIT failure) : " << eparam[iC] << " " ;
//     }
//     std::cout << std::endl;
//     for (int ihit=0;ihit<nfit;++ihit)
//       std::cout << iDet[ihit] << " " << ihit << " " << chi2hit[ihit] << std::endl;
//    std::cout << std::endl;
...
//   std::cout << std::endl;
//   std::cout << " # of point = " << nfit << std::endl;
//   std::cout << " fit = " << rfit[0] << " " << rfit[1] << " " << rfit[2] << " " 
//   	    <<  rfit[3] << " " << rfit[4] << " " << rfit[5] << " " 
//   	    << ierr << " " << chi2/float(ndf) << std::endl;
//   //  std::cout << std::endl;
...
//      std::cout << "warning : measurement code = 0" << std::endl;  
      pos[0] = 10*rfit[0];
      pos[1] = 10*rfit[1];
//    exit(1);
  }
...
//   std::cout << " param = " << param[0] << " " << param[1] << " " << param[2] << " " 
//  	    <<  param[3] << " " << param[4] << std::endl;
//   std::cout << std::endl;
//   HelixClass helixS;
//   helix.Initialize_Canonical(param[2],param[3],param[4],
// 			     param[0],param[1],bField);
  
//   std::cout << "helix = " << helix.getD0() << " "
// 	    <<  helix.getZ0() << " "    
// 	    <<  helix.getMomentum()[0] << " "
//     	    <<  helix.getMomentum()[1] << " "
//     	    <<  helix.getMomentum()[2] << " " << std::endl;
...
void MarlinTrackFit::CrudeErrorEstimates(float BField, float * reso, float * param, float * eparam) {
  ...
  // (D0,D0)
  eparam[9] = reso[2]*reso[2];

  // (Z0,Z0)
  eparam[14] = reso[3]*reso[3];
